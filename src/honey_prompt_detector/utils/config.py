...
class Config:
    ...
    def _load_configuration(self) -> None:
        ...
        # Load self-tuner configuration
        self.max_fp_rate = float(os.getenv("MAX_FP_RATE", "0.1"))  # Maximum acceptable false positive rate
        self.max_fn_rate = float(os.getenv("MAX_FN_RATE", "0.1"))  # Maximum acceptable false negative rate
        ...
        # Build alert settings with optional email and Slack integration
        self.alert_settings = {
        ...
        if os.getenv("SMTP_SERVER") and os.getenv("EMAIL_FROM") and os.getenv("EMAIL_TO"):
            self.alert_settings["email_settings"] = {
        ...
        if os.getenv("SLACK_WEBHOOK"):
            self.alert_settings["slack_webhook"] = os.getenv("SLACK_WEBHOOK")
    ...
    def _parse_log_level(self, level_str: str) -> int:
        ...
        if level_str not in level_mapping:
            raise ConfigurationError(
                f"Invalid log level: {level_str}. " f"Must be one of: {', '.join(level_mapping.keys())}"
            )
