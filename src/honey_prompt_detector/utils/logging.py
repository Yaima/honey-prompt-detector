...
def setup_logger(
    name: str, log_file: Optional[Path] = None, level: int = logging.INFO, retention_days: int = 30
) -> logging.Logger:
    """
    ...
    formatter = SecurityFormatter(
        "%(created_fmt)s %(process_thread)s %(levelname)s " "[%(source_location)s] %(name)s: %(message)s"
    )
    ...
    if log_file:
        ...
        # Set up log rotation with TimedRotatingFileHandler
        from logging.handlers import TimedRotatingFileHandler

        rotating_handler = TimedRotatingFileHandler(log_file, when="midnight", interval=1, backupCount=retention_days)
