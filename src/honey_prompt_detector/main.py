...
class HoneyPromptSystem:
    def __init__(self, env_path: Optional[Path] = None, custom_config: Optional[Dict[str, Any]] = None):
        ...
        self.token_designer = TokenDesignerAgent(api_key=self.config.openai_api_key, model_name=self.config.model_name)
        self.context_evaluator = ContextEvaluatorAgent(
            api_key=self.config.openai_api_key, model_name=self.config.model_name
        )
        ...
        self.orchestrator = Orchestrator(
            token_designer=self.token_designer, context_evaluator=self.context_evaluator, config=self.config
        )
    ...
    async def start(self) -> bool:
        try:
            # Start all agents with autonomous behavior
            logger.info("Starting TokenDesignerAgent...")
            await self.token_designer.start()

            logger.info("Starting ContextEvaluatorAgent...")
            await self.context_evaluator.start()

            logger.info("Starting EnvironmentAgent...")
            await self.orchestrator.environment_agent.start()

            # Initialize system (creates honey prompts)
            await self.orchestrator.initialize_system()

            self.is_initialized = True
            self.metrics.record_system_start()
            self._metrics_task = asyncio.create_task(self.periodic_save_metrics())
            logger.info("System started successfully - all agents running autonomously")
            return True
    ...
    async def monitor_text(self, text: str, expected_detection: Optional[bool] = None) -> Dict[str, Any]:
        ...
        try:
            sanitized_inputs = await self.orchestrator.environment_agent.sanitize_external_inputs(
                external_inputs=[text], honey_prompts=self.orchestrator.honey_prompts
            )
    ...
    async def stop(self):
        ...
        if self.is_initialized:
            ...
            # Cancel periodic metrics saving task
            ...
            await self.token_designer.stop()
            await self.context_evaluator.stop()
            await self.orchestrator.environment_agent.stop()
