...
class MetricsCollector:
    ...
    def reset_metrics(self):
        ...
        self.metrics = {
            ...
            "performance": {"avg_response_time": 0.0, "total_requests": 0, "errors": 0},
            "patterns": {
    ...
    def record_performance(self, response_time: float, is_error: bool = False):
        ...
        self.metrics["performance"]["avg_response_time"] = (avg_time * total + response_time) / (total + 1)
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        false_positive_rate = (
            self.metrics["detections"]["false_positives"] / total_detections if total_detections > 0 else 0
        )
    ...
    def get_common_patterns(self, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        patterns = sorted(self.metrics["patterns"]["common_contexts"].items(), key=lambda x: x[1], reverse=True)
        return [{"pattern": p[0], "count": p[1]} for p in patterns[:limit]]
    ...
    def load_metrics(self):
        ...
        if self.metrics_file and self.metrics_file.exists():
            with open(self.metrics_file) as f:
                ...
                # Convert all defaultdict-backed fields
                saved_metrics["detections"]["by_type"] = defaultdict(
                    int, saved_metrics["detections"].get("by_type", {})
                )
                saved_metrics["detections"]["by_confidence"] = defaultdict(
                    int, saved_metrics["detections"].get("by_confidence", {})
                )
                saved_metrics["patterns"]["common_contexts"] = defaultdict(
                    int, saved_metrics["patterns"].get("common_contexts", {})
                )
                # Normalize time_distribution hour keys back to int and restore defaultdict
                raw_time_dist = saved_metrics["patterns"].get("time_distribution", {})
                normalized_time_dist = {int(k): v for k, v in raw_time_dist.items()}
                saved_metrics["patterns"]["time_distribution"] = defaultdict(int, normalized_time_dist)
                # Restore defaultdict for token_effectiveness
                saved_metrics["patterns"]["token_effectiveness"] = defaultdict(
                    float, saved_metrics["patterns"].get("token_effectiveness", {})
                )
