...
class AlertManager:
    ...
    async def send_alert(
        self, detection_info: Dict[str, Any], additional_context: Optional[Dict[str, Any]] = None
    ) -> bool:
        """
    ...
    def _create_alert_message(
        self, detection_info: Dict[str, Any], alert_level: str, additional_context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
    ...
    async def _send_slack_alert(self, alert_message: Dict[str, Any], webhook_url: str, alert_level: str) -> None:
        ...
        try:
            detection_info = alert_message.get("detection_info", {})
            slack_message = {
                "attachments": [
                    {
                        ...
                        "fields": [
                            {
                                "title": "Confidence",
                                "value": f"{detection_info.get('confidence', 0):.2f}",
                                "short": True,
                            },
                            {
                                "title": "Match Type",
                                "value": detection_info.get("match_type", "unknown"),
                                "short": True,
                            },
                        ],
                    }
...
Confidence: {detection_info.get('confidence', 0):.2f}

Detection Details:
- Match Type: {detection_info.get('match_type', 'unknown')}
- Context: {detection_info.get('context', 'N/A')}
...
"""
    ...
    def _format_alert_for_slack(self, alert_message: Dict[str, Any]) -> str:
        ...
        return (
            ...
            f"*Confidence:* {detection_info.get('confidence', 0):.2f}\n"
            f"*Match Type:* {detection_info.get('match_type', 'unknown')}\n\n"
            f"*Context:*\n```{detection_info.get('context', 'N/A')}```"
        )
    ...
    def _record_alert(self, alert_message: Dict[str, Any], alert_level: str) -> None:
        ...
        self.alert_history.append(
            {"timestamp": alert_message["timestamp"], "level": alert_level, "message": alert_message}
        )
    ...
    async def get_recent_alerts(self, limit: int = 10, min_level: str = "LOW") -> List[Dict[str, Any]]:
        ...
        filtered_alerts = [
            alert for alert in self.alert_history if self.ALERT_LEVELS[alert["level"]]["threshold"] >= min_threshold
        ]
